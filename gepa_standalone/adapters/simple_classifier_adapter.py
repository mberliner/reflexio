        ...
        for i, (data, score) in enumerate(zip(source_data, eval_batch.scores)):
            if score < 1.0:
                ...
                if len(text) > max_len:
                    ...
                    print(f"[INFO] Texto truncado de {len(text)} a {max_len} caracteres para reflexión (Classifier)")
                else:
                    text_truncado = text
                ...
                reflective_record = {
                    ...
                    "Feedback": f"Clasificación incorrecta. Se esperaba '{exp}' pero se obtuvo '{pred}'."
                }
