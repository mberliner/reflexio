#!/bin/bash
#
# Script para ejecutar demos de GEPA Universal Optimizer.
# Lista los YAML de experiments/configs/, permite seleccionar cuales ejecutar
# y cuantas veces, y entrega un resumen final.
#

set -e

# Salida en orden y en tiempo real cuando stdout NO es una terminal (p.ej.
# ./run_demos_gepa.sh > log 2>&1 o | tee). Conviven dos buffers distintos:
...
# Numero de veces que se ejecutara cada configuracion (valor por defecto;
...
ejecutar_prueba() {
    ...
    echo ">>> Ejecutando: python -m gepa_standalone.universal_optimizer --config $config_file"
    echo ""
    echo "--- INICIO OUTPUT PRUEBA ---"
    local exit_code=0
    (cd "$REPO_ROOT" && run_py python -m gepa_standalone.universal_optimizer --config "$config_file") || exit_code=$?
...
clear
echo "=============================================="
echo "        GEPA Universal Optimizer Runner"
echo "=============================================="
echo ""
echo "  Configuracion:"
