...
class ReflexioDeclarativa:
    ...
    Replica la funcionalidad del optimizador universal pero adaptado a la arquitectura de Reflexio Nexus.
    """
    ...
    def setup_models(self):
        ...
        print(f"  Validando conexion con Task LM...")
        self.task_config.validate_connection()
        print(f"  [OK] Task LM conectado")
        ...
        print(f"  Validando conexion con Reflection LM...")
        self.reflection_config.validate_connection()
        print(f"  [OK] Reflection LM conectado")
    ...
    def load_data(self):
        ...
        self.trainset, self.valset, self.testset = loader.load_dataset(
            filename=self.config.raw_config['data']['csv_filename'],
            input_keys=[input_key]
        )
        print(f"Loaded {len(self.trainset)} training, {len(self.valset)} validation, and {len(self.testset)} test examples.")
    ...
    def create_module_and_metric(self):
        ...
        if module_type == "dynamic":
            ...
            if not sig_config:
                raise ValueError("Module type is 'dynamic' but no 'signature' section found in config.")
            
            predictor_type = self.config.raw_config.get('optimization', {}).get('predictor_type', 'cot')
            self.student = DynamicModuleFactory.create_module(sig_config, predictor_type=predictor_type)
            ...
            raise ValueError(f"Unsupported module type: {module_type}. Only 'dynamic' is supported in this version.")
    ...
    def _validate_metric_fields(self, eval_fields: list, output_fields: list) -> None:
        ...
        if invalid_fields:
            raise ConfigurationError(
                ...
                f"{'='*60}\n\n"
                f"Los siguientes campos en 'ignore_in_metric' o eval no existen en outputs:\n"
        ...
        if not eval_fields:
            print(f"  [WARN] No hay campos para evaluar. Todos los outputs estan en ignore_in_metric.")
...
def main():
    parser = argparse.ArgumentParser(description="Ejecutar optimización GEPA para DSPy (Reflexio Declarativa)")
