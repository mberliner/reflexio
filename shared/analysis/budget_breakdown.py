...
def run(
    ...
    for case_name, stats in sorted_cases:
        ...
        for combo, combo_data in sorted(stats['by_model_combo'].items(),
                                        key=lambda x: x[1]['cost'], reverse=True):
            percentage = (combo_data['cost'] / stats['total_cost'] * 100) if stats['total_cost'] > 0 else 0
            print(f"{combo:<45} {combo_data['count']:>8} {format_currency(combo_data['cost']):>15} {percentage:>7.1f}%")
    ...
    for case_name, stats in sorted(case_stats.items(),
        ...
        print(f"{case_name:<30} {stats['count']:>10} {format_currency(stats['total_cost']):>15} {percentage:>11.1f}%")
