...
- **GEPA no aporto en la 1a tanda, pero por ARTEFACTO de metrica, no por overfit**
  (correccion al primer analisis): la metrica evaluaba las 7 salidas (P1..P5 + alto_impacto
  + color), y como P1/P2/P3/P5 ya estaban en 90-100%, el promedio inflaba el baseline VAL a
  93-96% (techo) y diluia el cuello (color y alto_impacto pesan 1/7 cada uno). GEPA quedaba
  sin margen ni presion sobre lo que importa. NO es el patron de VAL chico de la seccion 11.
  Fix: metrica = `clasificacion` + `alto_impacto` (se ignoran P1..P5, ya altos); re-corrida
  de GEPA pendiente con el baseline destechado. La palanca confirmada hasta aca fue el
  afilado manual de descripciones + few-shot.
