...
| `shared/utils/` | Utilidades operativas: `ci_local.sh` (replica local del CI: lint, security, tests), `check_deployments.py`, `check_runs.py`, `seed_protocol.py`, `build_cv_v2_datasets.py` (generador de datasets), `keep_awake.sh`/`keep_awake.ps1` (anti-suspension) |
| `shared/cv_synth/` | Generador modular y reproducible de CVs sinteticos extensos. `reference_modelos_cv.txt` es el corpus de referencia estilistico |
